...
import psycopg2
conn = psycopg2.connect("dbname=tweets user=postgres password=zenvisage")

cursor = conn.cursor()
cursor.execute("""SELECT table_name FROM information_schema.tables
       WHERE table_schema = 'public'and table_name like '%sentiment%' """) # and table_name like '%diabetes%'
...
if __name__ == "__main__":
    ...
    top_k = 10
    #atr = ['race', 'gender', 'age']
    atr = ['response']#, 'cp', 'fbs', 'restecg', 'exang', 'thal', 'num'] # no , 'slope'
    #measure = ['time_in_hospital', 'num_lab_procedures', 'num_procedures']
    measure = ['*']#, 'bad_reason_confidence']
